...
</li>`).join(" ")}const p=document.querySelector(".search-form"),d=p.elements.search,u=document.querySelector(".gallery"),L=document.querySelector(".loader"),f=new h(".gallery-item a",{captionsData:"alt",captionDelay:250,showCounter:!1});function l(){L.classList.toggle("hidden")}function S(){u.innerHTML&&(u.innerHTML="")}function c(t){m.error({title:"Error",message:`${t}`})}p.addEventListener("submit",b);function b(t){if(t.preventDefault(),S(),l(),!d.value.trim()){l(),c("Please enter something!");return}g(d.value).then(s=>{if(!s.hits.length){c("Sorry, there are no images matching your search query. Please try again!");return}u.innerHTML=y(s.hits),f&&f.refresh()}).catch(s=>{c("Sorry, something wrong happened while fetching."),console.log(s)}).finally(()=>{l()})}
